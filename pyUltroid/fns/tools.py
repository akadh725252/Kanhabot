...
def telegraph_client():
    ...
    gd_name = kanha_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{kanha_bot.me.username}"
        if kanha_bot.me.username
        else "https://t.me/Teamkanha"
    )
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            TelegraphClient.create_account(
                short_name="kanhauser", author_name=gd_name, author_url=profile_url
            )
        else:
            LOGS.exception(er)
            return
...
async def Carbon(
    ...
    **kwargs,
    ...
    if rayso:
        ...
        kwargs["title"] = kwargs.get("title", "kanha")
    else:
        kwargs["code"] = code
