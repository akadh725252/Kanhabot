...
def load_addons(plugin_name):
    ...
    if base_name.startswith("__"):
        return
    from pykanha import fns

    from .. import HNDLR, LOGS, asst, udB, kanha_bot
    from .._misc import _supporter as config
    from .._misc._assistant import asst_cmd, callback, in_pattern
    from .._misc._decorators import kanha_cmd
    ...
    for path in configPaths:
        modules[path] = config
    modules["pykanha.functions"] = fns
    mod.LOG_CHANNEL = udB.get_key("LOG_CHANNEL")
    mod.udB = udB
    mod.asst = asst
    mod.tgbot = asst
    mod.kanha_bot = kanha_bot
    mod.ub = kanha_bot
    mod.bot = kanha_bot
    mod.kanha = kanha_bot
    mod.borg = kanha_bot
    mod.telebot = kanha_bot
    mod.jarvis = kanha_bot
    mod.friday = kanha_bot
    mod.eod = eod
    mod.edit_delete = eod
    mod.LOGS = LOGS
    ...
    mod.kanha_cmd = kanha_cmd
    mod.on_cmd = kanha_cmd
