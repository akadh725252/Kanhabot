...
class MongoDB(_BaseDatabase):
    def __init__(self, key, dbname="kanhaDB"):
        self.dB = MongoClient(key, serverSelectionTimeoutMS=5000)
        self.db = self.dB[dbname]
        super().__init__()

    def __repr__(self):
        return f"<kanha.MonGoDB\n -total_keys: {len(self.keys())}\n>"
...
# for SQL Implementation in kanha.
#
...
class SqlDB(_BaseDatabase):
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS kanha (kanhaCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('kanha')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'kanha'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM kanha")
    ...
    def set(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE kanha DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE kanha ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO kanha ({key}) values (%s)", (str(value),))
        return True

    def delete(self, key):
        try:
            self._cursor.execute(f"ALTER TABLE kanha DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE kanha")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS kanha (kanhaCli varchar(70))"
        )
        return True
