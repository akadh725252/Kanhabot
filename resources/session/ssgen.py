...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as kanha:
        ...
        print(er)
        print("If you think It as a Bug, Report to @kanhaSupportChat.\n\n")
...
def pyro_session():
    ...
    try:
        with Client(name="kanha", api_id=API_ID, api_hash=API_HASH, in_memory=True) as pyro:
            ss = pyro.export_session_string()
            pyro.send_message(
                "me",
                f"`{ss}`\n\nAbove is your Pyrogram Session String for @Thekanha. **DO NOT SHARE it.**",
            )
...
def main():
    ...
    try:
        type_of_ss = int(
            input(
                "\nkanha supports both telethon as well as pyrogram sessions.\n\nWhich session do you want to generate?\n1. Telethon Session.\n2. Pyrogram Session.\n\nEnter choice:  "
            )
        )
