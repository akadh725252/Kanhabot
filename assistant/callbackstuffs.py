...
try:
    from pykanha.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from pykanha.fns.helper import fast_download, progress
from pykanha.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from pykanha.startup.loader import Loader
...
_buttons = {
    "otvars": {
        "text": "Other Variables to set for @Teamkanha:",
        "buttons": [
            [
    ...
    "oofdm": {
        "text": "About [Dual Mode](https://t.me/kanhaUpdates/18)",
        "buttons": [
            [
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "pykanha")
...
async def changes(okk):
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• kanha Userbot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
    ...
    if len(changelog_str) > 1024:
        ...
        with open("kanha_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await okk.edit(
            get_string("upd_5"),
            file="kanha_updates.txt",
            buttons=button,
        )
        remove("kanha_updates.txt")
        return
    await okk.edit(
        changelog_str,
        buttons=button,
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Success!\nYou are all set to use Google Drive with kanha Userbot.`",
...
async def _(e):
    if not e.is_private:
        return
    await e.edit(
        "Click Authorise and send the code.\n\nYou can use your own CLIENT ID and SECRET by [this](https://t.me/kanhaUpdates/37)",
        buttons=[
            [
