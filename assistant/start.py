...
from pykanha._misc import SUDO_M, owner_and_sudos
...
**kanha [v{kanha_version}](https://github.com/Teamkanha/kanha), powered by @Teamkanha**
"""
...
async def own(event):
    msg = Owner_info_msg.format(
        mention=event.sender.mention, me=inline_mention(kanha_bot.me)
    )
    if custom_info:
...
async def kanha(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ok = ""
        me = inline_mention(kanha_bot.me)
        ...
        if not udB.get_key("STARTMSG"):
            ...
            await event.reply(
                f"Hey there {mention}, this is kanha Assistant of {me}!\n\n{ok}",
