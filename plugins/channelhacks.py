...
from . import LOGS, asst, eor, events, get_string, udB, kanha_bot, kanha_cmd
...
async def source(e):
    ...
    if not SourceM.contains(y):
        ...
        kanha_bot.add_handler(autopost_func, events.NewMessage())
    else:
        await e.eor(get_string("cha_3"))


@kanha_cmd(pattern="dsource( (.*)|$)")
async def dd(event):
...
@kanha_cmd(pattern="listdest")
async def list_all(event):
    kanha_bot = event.client
    x = await event.eor(get_string("com_1"))
    channels = DestiM.get()
    num = len(channels)
    if not num:
    ...
    for channel in channels:
        name = ""
        try:
            name = get_display_name(await kanha_bot.get_entity(int(channel)))
        except BaseException:
            name = ""
        msg += f"\n=> **{name}** [`{channel}`]"
    msg += f"\nTotal {num} channels."
