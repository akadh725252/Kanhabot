...
from pykanha.dB import stickers
from pykanha.dB.echo_db import check_echo
from pykanha.dB.forcesub_db import get_forcesetting
from pykanha.dB.gban_mute_db import is_gbanned
from pykanha.dB.greetings_db import get_goodbye, get_welcome, must_thank
from pykanha.dB.nsfw_db import is_profan
from pykanha.fns.helper import inline_mention
from pykanha.fns.tools import async_searcher, create_tl_btn, get_chatbot_reply

try:
    from ProfanityDetector import detector
except ImportError:
    detector = None
from . import LOG_CHANNEL, LOGS, asst, get_string, types, udB, kanha_bot
from ._inline import something


@kanha_bot.on(events.ChatAction())
...
async def DummyHandler(ult):
    ...
    if (
        ...
        if not user.bot:
            ...
            try:
                await kanha_bot(GetParticipantRequest(int(joinchat), user.id))
            except UserNotParticipantError:
                await kanha_bot.edit_permissions(
                    ult.chat_id, user.id, send_messages=False
                )
                res = await kanha_bot.inline_query(
    ...
    if ult.user_joined or ult.added_by:
        ...
        # gbans and @kanhaBans checks
        if udB.get_key("ULTROID_BANS"):
            try:
                is_banned = await async_searcher(
                    "https://bans.kanha.tech/api/status",
                    json={"userId": user.id},
                    post=True,
                    re_json=True,
                )
                if is_banned["is_banned"]:
                    ...
                    await ult.respond(
                        f'**@kanhaBans:** Banned user detected and banned!\n`{str(is_banned)}`.\nBan reason: {is_banned["reason"]}',
                    )
...
@kanha_bot.on(events.Raw(types.UpdateUserName))
