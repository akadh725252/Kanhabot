...
from pykanha.fns.helper import download_file
from pykanha.fns.tools import get_google_images

from . import LOGS, get_help, get_string, udB, kanha_bot, kanha_cmd

__doc__ = get_help("help_autopic")


@kanha_cmd(pattern="autopic( (.*)|$)")
async def autopic(e):
...
if search := udB.get_key("AUTOPIC"):
    ...
    async def autopic_func():
        ...
        file = await kanha_bot.upload_file(filee)
        await kanha_bot(UploadProfilePhotoRequest(file))
        os.remove(filee)
