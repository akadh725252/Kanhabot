...
from pykanha._misc._assistant import callback, in_pattern
from pykanha.dB._core import HELP, LIST
from pykanha.fns.helper import gen_chlog, time_formatter, updater
from pykanha.fns.misc import split_list
...
SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/Teamkanha/kanha"),
        Button.url("• Support •", url="t.me/kanhaSupportChat"),
    ],
]
...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="kanha Help Menu", text=text, buttons=_main_help_menu
        )
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("kanha Op", text=txt, link_preview=False, buttons=btn)
    ]
