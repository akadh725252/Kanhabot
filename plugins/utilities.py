...
from pykanha._misc._assistant import asst_cmd
from pykanha.dB.gban_mute_db import is_gbanned
from pykanha.fns.tools import get_chat_and_msgid
...
from pykanha.fns.info import get_chat_info
...
@kanha_cmd(pattern="kickme$", fullsudo=True)
async def leave(ult):
...
@kanha_cmd(pattern="paste( (.*)|$)", manager=True, allow_all=True)
async def _(event):
    try:
...
async def telegraphcmd(event):
    xx = await event.eor(get_string("com_1"))
    match = event.pattern_match.group(1).strip() or "kanha"
    reply = await event.get_reply_message()
    if not reply:
...
@kanha_cmd(pattern="suggest( (.*)|$)", manager=True)
...
async def get_restriced_msg(event):
    ...
    if not (chat and msg):
        return await event.eor(
            f"{get_string('gms_1')}!\nEg: `https://t.me/Teamkanha/3 or `https://t.me/c/1313492028/3`"
        )
    try:
