...
from pykanha.fns.admins import ban_time
from pykanha.fns.info import get_uinfo

from . import HNDLR, LOGS, eod, eor, get_string, inline_mention, types, kanha_cmd
...
@kanha_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
@kanha_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@kanha_cmd(pattern="pinned", manager=True, groups_only=True)
