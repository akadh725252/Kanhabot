...
async def pack_kangish(_):
    ...
    _packname = cmdtext or f"kanha Kang Pack By {_.sender_id}"
    typee = None
    if not local:
...
async def hehe(args):
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await kanha_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await kanha_bot.download_file(message.media.document, photo)
        if (
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent kanha_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with kanha_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await kanha_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await kanha_bot.send_read_acknowledge(conv.chat_id)
        else:
...
async def ultdestroy(event):
    ...
    await event.client.download_media(ult, "kanha.tgs")
    xx = await event.eor(get_string("com_1"))
    await bash("lottie_convert.py kanha.tgs json.json")
    ...
    file = await con.animated_sticker("json.json", "kanha.tgs")
    if file:
        await event.client.send_file(
            event.chat_id,
            file="kanha.tgs",
...
async def ultiny(event):
    ...
    im1 = Image.open("resources/extras/kanha_blank.png")
    if ik.endswith(".tgs"):
