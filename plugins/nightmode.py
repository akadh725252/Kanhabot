...
from . import get_string, udB, kanha_bot, kanha_cmd

keym = KeyManager("NIGHT_CHATS", cast=list)


@kanha_cmd(pattern="nmtime( (.*)|$)")
...
@kanha_cmd(pattern="addnm( (.*)|$)")
async def add_grp(e):
    if pat := e.pattern_match.group(1).strip():
        try:
            keym.add((await kanha_bot.get_entity(pat)).id)
...
@kanha_cmd(pattern="remnm( (.*)|$)")
async def r_em_grp(e):
    if pat := e.pattern_match.group(1).strip():
        try:
            keym.remove((await kanha_bot.get_entity(pat)).id)
...
@kanha_cmd(pattern="listnm$")
async def rem_grp(e):
    chats = keym.get()
    name = "NightMode Groups Are-:\n\n"
    for x in chats:
        try:
            ok = await kanha_bot.get_entity(x)
...
async def open_grp():
    for chat in keym.get():
        try:
            ...
            await kanha_bot.send_message(chat, "**NightMode Off**\n\nGroup Opened 🥳.")
