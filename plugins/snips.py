...
from pykanha._misc import sudoers
from pykanha.dB.snips_db import add_snip, get_snips, list_snip, rem_snip
from pykanha.fns.tools import create_tl_btn, format_btn, get_msg_button

from . import events, get_string, mediainfo, udB, kanha_bot, kanha_cmd
from ._inline import something


@kanha_cmd(pattern="addsnip( (.*)|$)")
async def an(e):
    ...
    kanha_bot.add_handler(add_snips, events.NewMessage())


@kanha_cmd(pattern="remsnip( (.*)|$)")
async def rs(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    if not wrd:
    ...
    for z in xx:
        if k := get_snips(z):
            ...
            if rep:
                ...
                await kanha_bot.send_message(e.chat_id, msg, file=media)
