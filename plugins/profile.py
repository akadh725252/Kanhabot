...
from . import eod, eor, get_string, mediainfo, kanha_cmd
...
@kanha_cmd(pattern="setbio( (.*)|$)", fullsudo=True)
async def _(ult):
    ok = await ult.eor("...")
...
@kanha_cmd(pattern="setname ?((.|//)*)", fullsudo=True)
async def _(ult):
    ok = await ult.eor("...")
...
@kanha_cmd(pattern="setpic$", fullsudo=True)
async def _(ult):
    if not ult.is_reply:
...
@kanha_cmd(pattern="delpfp( (.*)|$)", fullsudo=True)
