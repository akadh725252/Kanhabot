...
from pykanha import _ignore_eval
...
try:
    ...
    from pykanha.fns.tools import safe_load
try:
...
@kanha_cmd(pattern="bash", fullsudo=True, only_devs=True)
async def _(event):
...
@kanha_cmd(pattern="eval", fullsudo=True, only_devs=True)
async def _(event):
    try:
        cmd = event.text.split(maxsplit=1)[1]
    except IndexError:
    ...
    if any(item in cmd for item in KEEP_SAFE().All) and (
        not (event.out or event.sender_id == kanha_bot.uid)
    ):
...
@kanha_cmd(pattern="cpp", only_devs=True)
async def doie(e):
    ...
    open("cpp-kanha.cpp", "w").write(match)
    m = await bash("g++ -o Cppkanha cpp-kanha.cpp")
    o_cpp = f"• **Eval-Cpp**\n`{match}`"
    if m[1]:
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
            os.remove("cpp-kanha.cpp")
            if os.path.exists("Cppkanha"):
                os.remove("Cppkanha")
