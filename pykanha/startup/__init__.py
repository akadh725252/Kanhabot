...
if run_as_module:
    ...
    from ..version import __version__ as __pykanha__
    from ..version import kanha_version

    file = f"kanha{sys.argv[6]}.log" if len(sys.argv) > 6 else "kanha.log"
    ...
    LOGS.info(f"py-kanha Version - {__pykanha__}")
    LOGS.info(f"Telethon Version - {__version__} [Layer: {LAYER}]")
    LOGS.info(f"kanha Version - {kanha_version} [{HOSTED_ON}]")
