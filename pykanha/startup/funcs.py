...
async def autoupdate_local_database():
    from .. import Var, asst, udB, kanha_bot

    global db_url
    db_url = (
        udB.get_key("TGDB_URL") or Var.TGDB_URL or kanha_bot._cache.get("TGDB_URL")
    )
    if db_url:
        _split = db_url.split("/")
        _channel = _split[-2]
...
async def startup_stuff():
    ...
    if TZ and timezone:
        try:
            ...
            LOGS.critical(
                "Incorrect Timezone ,\nCheck Available Timezone From Here https://graph.org/kanha-06-18-2\nSo Time is Default UTC"
            )
            os.environ["TZ"] = "UTC"
            time.tzset()
...
async def autobot():
    from .. import udB, kanha_bot

    if udB.get_key("BOT_TOKEN"):
        return
    await kanha_bot.start()
    ...
    if who.username:
        ...
        username = "kanha_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    await kanha_bot(UnblockRequest(bf))
    await kanha_bot.send_message(bf, "/cancel")
    await asyncio.sleep(1)
    await kanha_bot.send_message(bf, "/newbot")
    await asyncio.sleep(1)
    isdone = (await kanha_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("That I cannot do.") or "20 bots" in isdone:
        ...
        sys.exit(1)
    await kanha_bot.send_message(bf, name)
    await asyncio.sleep(1)
    isdone = (await kanha_bot.get_messages(bf, limit=1))[0].text
    if not isdone.startswith("Good."):
        await kanha_bot.send_message(bf, "My Assistant Bot")
        await asyncio.sleep(1)
        isdone = (await kanha_bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
            ...
            sys.exit(1)
    await kanha_bot.send_message(bf, username)
    await asyncio.sleep(1)
    isdone = (await kanha_bot.get_messages(bf, limit=1))[0].text
    await kanha_bot.send_read_acknowledge("botfather")
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "kanha_" + (str(who.id))[6:] + str(ran) + "_bot"
        await kanha_bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await kanha_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("Done!"):
        ...
        await enable_inline(kanha_bot, username)
        LOGS.info(
...
async def autopilot():
    from .. import asst, udB, kanha_bot

    channel = udB.get_key("LOG_CHANNEL")
    new_channel = None
    if channel:
        try:
            chat = await kanha_bot.get_entity(channel)
    ...
    if not channel:
        ...
        async def _save(exc):
            udB._cache["LOG_CHANNEL"] = kanha_bot.me.id
            await asst.send_message(
                kanha_bot.me.id, f"Failed to Create Log Channel due to {exc}.."
            )
    ...
    assistant = True
    try:
        await kanha_bot.get_permissions(int(channel), asst.me.username)
    except UserNotParticipantError:
        try:
            await kanha_bot(InviteToChannelRequest(int(channel), [asst.me.username]))
        except BaseException as er:
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        ...
        ll = await kanha_bot.upload_file(photo)
        try:
            await kanha_bot(
...
async def customize():
    ...
    try:
        ...
        if not kanha_bot.me.username:
            sir = kanha_bot.me.first_name
        else:
            sir = f"@{kanha_bot.me.username}"
        file = random.choice(
            [
                "https://graph.org/file/92cd6dbd34b0d1d73a0da.jpg",
                "https://graph.org/file/a97973ee0425b523cdc28.jpg",
                "resources/extras/kanha_assistant.jpg",
            ]
        )
        ...
        await asyncio.sleep(1)
        await kanha_bot.send_message("botfather", "/cancel")
        await asyncio.sleep(1)
        await kanha_bot.send_message("botfather", "/setuserpic")
        await asyncio.sleep(1)
        isdone = (await kanha_bot.get_messages("botfather", limit=1))[0].text
        if isdone.startswith("Invalid bot"):
            LOGS.info("Error while trying to customise assistant, skipping...")
            return
        await kanha_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await kanha_bot.send_file("botfather", file)
        await asyncio.sleep(2)
        await kanha_bot.send_message("botfather", "/setabouttext")
        await asyncio.sleep(1)
        await kanha_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await kanha_bot.send_message(
            "botfather", f"✨ Hello ✨!! I'm Assistant Bot of {sir}"
        )
        await asyncio.sleep(2)
        await kanha_bot.send_message("botfather", "/setdescription")
        await asyncio.sleep(1)
        await kanha_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await kanha_bot.send_message(
            "botfather",
            f"✨ Powerful kanha Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @Teamkanha ✨",
        )
        await asyncio.sleep(2)
...
async def plug(plugin_channels):
    ...
    for chat in plugin_channels:
        LOGS.info(f"{'•'*4} {chat}")
        try:
            async for x in kanha_bot.iter_messages(
                ...
                if not os.path.exists(plugin):
                    ...
                    try:
                        ...
                        LOGS.info(f"kanha - PLUGIN_CHANNEL - ERROR - {plugin}")
                        LOGS.exception(e)
                        os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
...
async def fetch_ann():
    ...
    try:
        updts = await async_searcher(
            "https://kanha-api.vercel.app/announcements", post=True, re_json=True
        )
        for upt in updts:
            key = list(upt.keys())[0]
            if key not in get_:
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """🎇 **Thanks for Deploying kanha Userbot!**
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://graph.org/file/54a917cc9dbb94733ea5f.jpg"
        BTTS = Button.inline("• Click to Start •", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**kanha has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(kanha_bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n**Support**: @Teamkanha\n➖➖➖➖➖➖➖➖➖➖"
        BTTS, PHOTO = None, None
        prev_spam = udB.get_key("LAST_UPDATE_LOG_SPAM")
        if prev_spam:
            try:
                await kanha_bot.delete_messages(chat_id, int(prev_spam))
            except Exception as E:
    ...
    try:
        ...
        try:
            await (await kanha_bot.send_message(chat_id, str(e))).delete()
            spam_sent = await asst.send_message(chat_id, MSG, file=PHOTO, buttons=BTTS)
        ...
        LOGS.info(el)
        try:
            spam_sent = await kanha_bot.send_message(chat_id, MSG)
...
async def WasItRestart(udb):
    ...
    if not key:
        return
    from .. import asst, kanha_bot

    try:
        data = key.split("_")
        who = asst if data[0] == "bot" else kanha_bot
...
async def enable_inline(kanha_bot, username):
    bf = "BotFather"
    await kanha_bot.send_message(bf, "/setinline")
    await asyncio.sleep(1)
    await kanha_bot.send_message(bf, f"@{username}")
    await asyncio.sleep(1)
    await kanha_bot.send_message(bf, "Search")
    await kanha_bot.send_read_acknowledge(bf)
