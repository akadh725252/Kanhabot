...
def load_other_plugins(addons=None, pmbot=None, manager=None, vcbot=None):
    ...
    if addons:
        ...
        if not os.path.exists("addons"):
            subprocess.run(
                f"git clone -q -b {Repo().active_branch} https://github.com/Teamkanha/kanhaAddons.git addons",
                shell=True,
            )
        else:
        ...
        if not os.path.exists("addons"):
            subprocess.run(
                "git clone -q https://github.com/Teamkanha/kanhaAddons.git addons",
                shell=True,
            )
    ...
    if vcbot and not vcClient._bot:
        try:
            ...
            if not os.path.exists("vcbot"):
                subprocess.run(
                    "git clone https://github.com/Teamkanha/VcBot vcbot", shell=True
                )
            try:
