...
def main():
    ...
    if (
        ...
        and kanha_bot.run_in_loop(updater())
    ):
        kanha_bot.run_in_loop(bash("bash installer.sh"))

        os.execl(sys.executable, sys.executable, "-m", "pykanha")

    kanha_bot.run_in_loop(startup_stuff())

    kanha_bot.me.phone = None

    if not kanha_bot.me.bot:
        udB.set_key("OWNER_ID", kanha_bot.uid)

    LOGS.info("Initialising...")

    kanha_bot.run_in_loop(autopilot())
    ...
    suc_msg = """
            ----------------------------------------------------------------------
                kanha has been deployed! Visit @Thekanha for updates!!
            ----------------------------------------------------------------------
    """
    ...
    # Customize kanha Assistant...
    kanha_bot.run_in_loop(customize())

    # Load Addons from Plugin Channels.
    if plugin_channels:
        kanha_bot.run_in_loop(plug(plugin_channels))

    # Send/Ignore Deploy Message..
    if not udB.get_key("LOG_OFF"):
        kanha_bot.run_in_loop(ready())
    ...
    kanha_bot.run_in_loop(WasItRestart(udB))
