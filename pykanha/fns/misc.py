...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=kanha_bot
):
...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="kanha All Cmds", content=[txt])
    await eor(event, f"All kanha Cmds : [Click Here]({t['url']})", link_preview=False)
