...
from pykanha.exceptions import DependencyMissingError
...
from ..version import kanha_version as ult_ver
...
def kanha_cmd(
    pattern=None, manager=False, kanha_bot=kanha_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                await kanha_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await kanha_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        ...
                        Button.url(
                            "Repl",
                            "https://replit.com/@Thekanha/kanhaStringSession",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = "**kanha Client Error:** `Forward this to` @kanhaSupportChat\n\n"
                ftext += "**Py-kanha Version:** `" + str(pyver)
                ftext += "`\n**kanha Version:** `" + str(ult_ver)
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**kanha Client Error:** `Forward this to` @kanhaSupportChat\n\n",
                        )
                else:
